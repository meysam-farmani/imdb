...
class SearchTitleViewModel(application: Application): AndroidViewModel(application) {

    constructor(application: Application, test: Boolean = true): this(application){
        iniected = true
    }

    val movieDataList by lazy { MutableLiveData<SearchTitle>() }
    ...
    fun search(expression: String?){
        ...
        getSearchTitle(apiKey, expression)
    }

    private fun getSearchTitle(apiKey: String, expression: String?){
        disposable.add(
            apiService.getSearchTitle(apiKey, expression)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(object: DisposableSingleObserver<SearchTitle>(){
                    override fun onSuccess(searchTitle: SearchTitle) {
                        loadError.value = false
                        movieDataList.value = searchTitle
                        loading.value = false
                    }
